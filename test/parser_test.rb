...
class DGDParserTest < Minitest::Test
  ...
  def test_functions
    ...
    assert_equal ["local_vars_should_parse", "test_args"], sf.func_decls.map { |fd| fd[:name] }
  end
  ...
  def test_skotos_lib_version_c
    ...
    assert_equal ["dgd_version"], sf.func_decls.map { |fd| fd[:name] }
  end
  ...
  def test_skotos_httpconn_c
    ...
    #  "state_responding", "state_virgin", "url_absolute" ], sf.func_decls.map { |fd| fd[:name] }.sort
  end
